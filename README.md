...
`EP.` numbers come from `data/episode-numbers.csv`, checked in and regenerated by
`scripts/schedule_numbers.py`. There is nowhere else to get them: the feed carries no
`itunes:episode` tag, the show stopped printing the number in its titles in January 2024, and
they cannot be counted out — a few episodes take no number at all. The private IRCB Schedule
workbook is the only source.

The update job refreshes the CSV on its own, from an Apps Script web app that serves the
workbook's numbering columns as JSON. `scripts/schedule-webapp.gs` is the script and carries
its own deploy steps; the deployment URL, with its token, is the `SCHEDULE_WEBAPP_URL`
repository secret. Without that secret — in a fork, say — the step is skipped and the
committed CSV stands, exactly as it did before.

It reads through the Sheets API advanced service rather than `SpreadsheetApp`, which is the
only way to hold the grant at `spreadsheets.readonly`: `SpreadsheetApp` refuses to run under
that scope and requires read-write on every sheet the account can reach. Adding the service
from the editor's **Services** menu also switches the Sheets API on in the script's Cloud
project — calling the same API by hand through `UrlFetchApp` fails with a 403 until you enable
it in the console yourself. Two consequences of the API worth knowing before editing it — trailing empty cells are omitted rather than padded, so
every column index may be past the end of a row, and dates arrive as the serial number the
sheet stores, which is what keeps timezones out of the answer entirely.

Refreshing it by hand still works and is the fallback when the web app does not answer. Pull
the workbook from Drive as **xlsx**, because the native export truncates it and drops rows
silently, then:
