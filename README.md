...
Each episode carries `<itunes:keywords>`, and `data/tag-taxonomy.json` types every distinct
term — `series`, `publisher`, `creator`, `topic`, and a few show-specific kinds. It is
generated by `scripts/build-tag-taxonomy.mjs` from `data/tag-seeds.json`, which is the
hand-curated half; correct a wrong call by editing the seeds and re-running, because
hand-edits to the taxonomy are lost.
...
The taxonomy is a checked-in artifact, so terms from episodes that aired after the last
`build-tag-taxonomy.mjs` run are unclassified until it is re-run. Those episodes still get
chip links for any term that spells its own heading, because the view falls back to matching
the episode's own mentions, but they contribute no tagged mentions.
